...
export function CustomerRegisterPage({ onBack, onLoginClick, onRegisterSuccess }: CustomerRegisterPageProps) {
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    ...
    try {
      const response = await authCustomerApi.requestOTP(formData.email);
      if (response.code === 200) {
        // Open OTP popup on success
  ...
  const handleOTPVerify = async (otpCode: string) => {
    // Verify OTP
    const verifyResponse = await authCustomerApi.verifyOTP(formData.email, otpCode);
    ...
    try {
      ...
      registerData.verify_token = verifyResponse.data;
      
      const registerResponse = await authCustomerApi.signup(registerData);
